...
import complexes_visualize_dev_v21_c_v02
        ...
        if s != '':
            ...
            try:
                if source['file_type'] == 'species':
                    source_path = shutil.copyfile(s, des + source['file_name'] + '.species')
                    visualize = complexes_visualize_dev_v21_c_v02.complexes_vixualize(source_path,
                                                                                      source['file_name'],
                                                                                      des)
                    if visualize:
                        complexes_visualize_dev_v21_c_v02.view(des + source['file_name'] + '_visualize.html')
                        return True, source['file_name'], 'Species'

                elif source['file_type'] == '0':
                    source_path = des + source['file_name'] + '.species'
                    dump_to_species_converter_v25.dump_to_species(s, source_path, source['file_name'])
                    visualize = complexes_visualize_dev_v21_c_v02.complexes_vixualize(source_path,
                                                                                      source['file_name'],
                                                                                      des)
                    if visualize:
